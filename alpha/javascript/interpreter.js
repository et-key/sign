 ...
 *
 * 【この冒頭は3つ嘘をついていた】記録として残す——**ファイルの顔は最も古びやすい**。
 * 機能を足したとき、足した場所のコメントは直しても、冒頭の「できないこと」の一覧は
 * 誰も読み返さない。書いた時点では正しかったものが、そのまま制限の宣言として残る。
 *
 *   「`$`/`@`/`#` は未対応」  → 実装済み（`n : 5` / `$n # 99` / `n` は 99）
 *   「TCO は行わない」        → 実装済み（末尾再帰 200000 段が通る。applyClosure の
 *                              トランポリン。tco.md は最適化ではなく言語仕様としている）
 *   「`!=` のみ右辺 Unit が例外」→ `!=` も `!==` も**左右対称**（どちらの辺が Unit でも
 *                              非 Unit 側を返す）
 *
 * 【中核の意味論】
 * - 完全性公理（type_system.md §3.4）：`f __ = __`。引数のいずれかが Unit（デフォルトも
 *   `rest~` のフォールバックも無い場合）なら、本体を評価せず即座に Unit を返す。
 * - デフォルト引数への Unit フォールバック（§3.4）：デフォルトを持つ仮引数に Unit が
 *   渡されたら、デフォルト式を（let* 的に、直前までの束縛を使って）評価する。
 * - rest への Unit フォールバック（§3.3）：rest の実引数が無いか Unit 単体なら空リストへ
 *   落ちる（完全性公理による崩壊は起きない）。
 * - 算術の Unit 規則：**両辺とも単位元**。`__` は積と余積の単位元なので、片方が `__` なら
 *   もう片方が通り抜ける（`2 * __` は 2）。**ただし非可換な左辺は未決**——`__ - 2` は
 *   単位元を代入すれば `0 - 2` で -2 になるはずだが、実装は 2 を返す。
 * - 比較の吸収則：両辺とも吸収元（`5 < __` も `__ < 5` も `__`）。`!=`/`!==` だけが
 *   例外で、こちらは両辺とも恒等射（非 Unit 側を返す）。
 * - `&`/`|`/`;` の短絡：`&` は左辺が Unit なら右辺を評価せず Unit、`|` は左辺が非 Unit
 *   なら右辺を評価せず左辺を返す。
 * - 多引数の一括適用：pass2 の applyChainInfo と対称に、apply 連鎖を遡って引数を全部
 *   集めてから一度だけ本体を評価する（中間クロージャを作らない）。
 * - 未定義識別子の Unit 収束（unit.md §0.1）：どのスコープにも無い識別子は例外を投げず
 *   `__` へ落ち、非ブロッキングな "information" として `env.diagnostics` に記録される。
 *   **これは「評価したかどうか」を測る唯一のオラクルでもある**——短絡の検査がここに
 *   乗っている（演算の副産物に乗せると、その演算を直したとき検査が黙って空振りになる）。
 */
...
function constructValues(node, l, r) {
        ...
        //
        // **左の器へ入れる。** `a~ b~` は「a の器に b の器の中を入れる」であって、両方から
        // 新しい器を作る操作ではない。だから結果は `a` そのもの——`a` を見ている他の誰かにも
        // 見える。これが `this~ that~` を上書きとして成立させている一点である。
        //
        // **上書きになるか複写になるかは、左が何かで決まる。** 左が名前なら既にある器なので
        // 上書き、左がその場のリテラル（`[foo : 99]~ this~`）ならその器は今作られたものな
        // ので、そこへ入れることがそのまま複写になる。**同じ規則で両方が出る**ので、
        // 例外を書かなくてよい。
        //
        // 新しい器を返していたので、上書きの側が成立していなかった。
        {
          const lo = isIterator(l) && isNamedSlots(l.origin) ? l.origin : l;
          if (isIterator(r) && isNamedSlots(r.origin) && isNamedSlots(lo)) {
            return Object.assign(lo, r.origin);
          }
        }
