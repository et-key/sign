 ...
 *
 * `push` と `unshift` も余積である。ここで均す4つの名前（`construct` / `concat` /
 * `push` / `unshift`）は、どれも「列が1段の中で伸びる」ことを言っているにすぎず、
 * **どちら側から伸びたか**が違うだけである。**ただし `push` は pass2 が作らない**
 * ——外から与えた AST を受けるための入口語彙として残してある（`unshift` に均される）。
 * ここで `push` / `unshift` を均さないと、`[1 2] 3` のように名前を経由せず伸びた列だけ
 * 大きさが出せなくなる——文字列側（``ab` `cd``）が数えられるのと食い違う。
 *
