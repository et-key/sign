...
function computeAtomType(node, env) {
  ...
  if (node.type === "operation") {
    ...
    if (node.operand) {
      ...
      //   列       前置 ~ が持ち上げ（→ 長さ1の List） 後置 ~ が持ち下げ（展開）
      //
      // **行き先は長さ1の器（`List`）であって「場所」ではない。** 以前はここが
      // `Implicit(T)` を返していたが、番地は表に出さないと決めた——詳しい理由は下の
      // 2030 行付近に書いてある（`Implicit` は要素型が1段ずれるので `~5 ' 0` が
      // 器そのものを返す、という実害も出ていた）。要素型は elementType に載せる。
      // **器なら恒等、スカラーなら持ち上げ**（`~x ≅ [x]`、冪等）。
