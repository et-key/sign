...
// 後置演算子か空白のときだけ式になる。それ以外はすべてコメントである。**
//
// 判別子を「後続があるか」から「閉じの直後が演算子か」へ狭めた。前者だと、散文の中で
// バッククォートを閉じただけの行まで式になってしまう——書き手から見れば明らかに
// コメントなのに、偶数個書くと構文エラーになっていた。言語の構文を説明するのが
// コメントの仕事である以上、その構文を引用できないのは実際に困る。
//
// 行頭から文字列で始まる式は、インポート（@）か連結（空白）である。散文では最初の
// 閉じバッククォートの直後は引用したい中身（非空白）なので、そこで分かれる。
//
// 否定先読みの中でバッククォート自身を文字クラスから除くのは変わらない。PEG の * は
// バックトラックしないので、除かないと走査が行末まで食い尽くして閉じを見つけられない。
comment = SOL "`" !([^\r\n`]* "`" [@~! ]) [^\r\n]* (EOL / EOF) { return null; }
