...
Block
  ...
  // ノルム（要素数）。**囲み記号のひとつ**であり、`[...]` などと同じ場所に住む。
  //
  // 2文字なのは `|` が中置 `or`（tier 6）で埋まっているからである——`|[...]|` にすると
  // `a | [1 2] | b` と区別がつかず、空白の有無で意味が変わってしまう。`~|` と `|~` は
  // どの演算子にもなっていない綴りなので、空きを取る。
  //
  // 絶対値（`|x|`）と分けるのは、**1要素の器が存在しない**からである。`[5] ≅ 5` なので
  // `|[5]|` を絶対値と読むと 5、要素数と読むと 1 になり、同じ記号が長さ1で意味を変える
  // ——`count : xs ? |xs|` が `[7]` に対して 7 を返していた。囲みを分ければどちらも
  // 曖昧さなく書ける（`|5|` は 5、`~|5|~` は 1）。
  / "~|" _ exprs:Expressions _ "|~" { return [`"NORM_"`, exprs]; }
...
// `~` が `|` に密着しているときは前置演算子ではない——ノルムの囲み（`~|...|~`）の開き
// である。ここで食ってしまうと、`Core`（＝`Block`）へ `~|` という綴りが届かない。
prefix
  = "###" / "##" / "#" / ("-" &(Block / identifier)) / $("~" !"|") / "!!" / "!" / "$" / "@"
...
// `|~` はノルムの閉じであって演算子ではない。ここで拾ってしまうと `Expressions` が
// 閉じまで食べてしまい、`~| xs |~` のように空白を空けた形が組み立てられない
// （密着した `~|xs|~` だけが通る、という不自然な仕様になっていた）。
operator
  = !"|~" op:$[!"#$%&'-=^~\|@;+:*,<>/?]+ { return op; }
