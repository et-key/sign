/**
 * Sign Language AST Helpers (alpha/javascript)
 * 
 * 役割:
 * 1. Hole (_) の脱糖 (desugarHoles)
 * 2. 識別子および環境情報の収集 (buildEnvironment, collectIdentifiers)
 * 3. ノードの型推論 (inferType)
 */
...
export function inferType(node, env) {
  ...
  if (node.type === 'block') {
    if (node.kind === 'bracket') return 'List';
  ...
  if (node.type === 'operation') {
    if (node.operator === ':') return inferType(node.right, env);
    ...
    if (node.operator === ' ') {
      ...
      if (node.name === 'apply') {
        ...
        if (leftType === 'Lambda') {
          ...
          if (appliedCount < arity) return 'Lambda';
