/**
 * Sign Language Coproduct Resolver (alpha/javascript)
 * 
 * 役割:
 * Pass 2 / Pass 3 において、Shunting Yard / PEG パーサーでフラットな `coproduct_block`
 * 配列として組み立てられた要素を、左辺・右辺の構文的カテゴリ（Lambda / Atom）に基づいて
 * 優先度順 (10.3 ~ 10.0) に左結合で還元 (Reduction) し、2分木 AST ノードへ変換する。
 */
...
function getCategory(node, env) {
  ...
  if (typeof node === 'string') {
    ...
    if (node.startsWith('`') && node.endsWith('`')) return 'Atom';
...
export function getParamCount(node) {
  ...
  if (node.type === 'operation') {
    ...
    if (node.operator === ':') return getParamCount(node.left);
    if (node.operator === '~') {
...
export function getArity(node, env) {
  ...
  if (node.type === 'coproduct_block') return 0;
  if (node.type === 'operation') {
    if (node.name === 'compose') return getArity(node.right, env);
    if (node.name === 'apply') {
      let appliedCount = 1;
    ...
    if (node.operator === '?') return getParamCount(node.left);
    if (node.position !== 'prefix' && node.position !== 'postfix') {
      if (node.left === undefined && node.right === undefined) return 2;
      if (node.left === undefined || node.right === undefined) return 1;
    }
    if (node.position === 'prefix' || node.position === 'postfix') {
      if (node.operand === '_' || node.operand === undefined) return 1;
    }
  }
  return 0;
}
