/**
 * Sign Language Operator Table (alpha/javascript)
 * 仕様書: documents/ja-jp/impl/syntax/operator_table.md
 */

// 逆引き用: 優先順位順の配列 (precedence順、1-indexed)
...
/**
 * 多義的演算子（複数の position を持つか enclosure）
 */
...
// QUESTION: 演算子 precedence 23 に '-' (negate) が入っているが、`-x` のような負の数リテラル/前置マイナスと
// 中置減算 `a - b` の決定は Lexer 段階ではなく Parse / Shunting Yard 段階で行う必要がある。
// 現在の separateInfix では `-` を多義的として自動空白挿入の対象外にしているが、これに異論はないか？
