...
//
// 終端の無い2項指定（`[1 ~+ 1]`）については、ここでは何も表明しない。これは
// Pull型の無限ストリームであり、type_system.md の Layer 2 が `Iterator(T)` を
// 持つ以上いずれ値として成立させるべきものだが、その意味論はまだ決めていない。
// 現在の評価器が例外を投げることを「そういう仕様」としてテストに書くと、
// 実装の都合が規範のように見えてしまう。決まってから書く。
...
// List / Struct は点ではないので端点にできない。型で分かるので原理4により静的に弾く。
// 検査が無かった頃は数値経路の `v + step` が値を伸ばし続け、100万回のガードに
// 当たるまで走っていた（実際に `[1 2] ~ [3 4]` がハングとして踏まれた）。
checkThrows("[1 2] ~ [3 4] → TypeError（List は範囲の端点になれない）", () => run("[1 2] ~ [3 4]"));
checkThrows("[x : 1] ~ [y : 2] → TypeError（Struct も同様）", () => run("[x : 1] ~ [y : 2]"));
// String が1文字かどうかは型では決まらないため、多文字は実行時に弾く。
checkThrows("`abc` ~ `xyz` → TypeError（多文字の String は点ではない）", () => run("`abc` ~ `xyz`"));
