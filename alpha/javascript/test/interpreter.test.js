...
// **同一型でない要素は足せない。** `List` は同じ幅の要素が並ぶ連続領域なので（§2）、
// 整数のリストへリストを足すのは不揃いであり、静的に拒否される。混ぜたいならカンマで
// `Struct` にする——これが「カンマは次元を上げる」ということでもある。
checkThrows("整数のリストへリストは足せない", () => run("[1 2] [3 4]"));
...
checkThrows("整数のリストへリストは足せない", () => run("[1 2 3] [1 2 3]"));
checkThrows("2要素でも同じ", () => run("[1 2] [3 4]"));
