...
// カッコの中のホールは、そのカッコの中でラムダになる。以前は脱糖されず
// `[_ ' 0] [3 , 4]` が `[3,4]` を返していた（ホールがただの穴として素通りしていた）。
check("[_ ' 0] [3 , 4] → 3（カッコがホールの作用域を区切る）", run("[_ ' 0] [3 , 4]"), 3);
check("[_ ' 1] [3 , 4] → 4", run("[_ ' 1] [3 , 4]"), 4);

// ただし get をポイントフリーで書くならホールは要らない——`[' 0]` が本来の形である。
// 以前は applyPointfree の combine が get_prop を知らず「未対応の演算子」で落ちていたため、
// `[_ ' 0]` というホールを使った回避形を書かざるを得なかった。
