...
// ---- ポイントフリーの合成が引数を貪欲に消費する ----
// `f g` は左→右のパイプライン（`(f g)(x) = g(f(x))`）なので実引数は**左の関数**へ渡る。
// 左が貪欲なポイントフリーなら合成全体も貪欲でなければならない。以前は
// isBarePointfreeChainBase が compose ノードを見落としており、合成へ引数を1個だけ渡して
// 残りを concat していた（`[* 2,] [+] 1 2 3 4 5` → `[2 2 3 4 5]`）。
check("[* 2,] [+] 1 2 3 4 5 → [2 4 6 8 10]（合成でも5引数すべてを写像）", run("[* 2,] [+] 1 2 3 4 5"), [2, 4, 6, 8, 10]);
check("[+] ([* 2,] 1 2 3 4 5)~ → 30（畳み込むには後置~で展開する）", run("[+] ([* 2,] 1 2 3 4 5)~"), 30);
