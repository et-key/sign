...
// `5 [+]` は、ポイントフリーが apply_reverse の対象外であるため縮約しきれない
...
// 直積（カンマ）は Unit を**スロットとして保持**する。零射でも恒等射でもない。
// `Struct` は固定オフセットで並ぶ連続ブロックであり（type_system.md §2）、スロットが値に
// よって消えると後続フィールドのオフセットが実行時の値に依存してしまうためである。
// 余積（空白）は逆に Unit を消す——連接の単位元だからである。
// この2演算子によって「空」（寄与しない）と「不在という値」（場所は残る）が既に分かれている。
checkTrue("[1 , __ , 2] は3スロット（Unitを保持）", (() => {
	const v = run("[1 , __ , 2]");
	return Array.isArray(v) && v.length === 3 && isUnit(v[1]);
})());
check("|[1 , __ , 2]| → 3", run("|[1 , __ , 2]|"), 3);
check("[1 , __ , 2] ' 2 → 2（オフセットがずれない）", run("[1 , __ , 2] ' 2"), 2);
checkTrue("名前付きスロットも同じくキーが残る", (() => {
	const v = run("s : [\n\ta : 1\n\tb : __\n\tc : 3\n]\ns");
	return v && typeof v === "object" && Object.prototype.hasOwnProperty.call(v, "b") && isUnit(v.b);
})());
check("s ' c → 3（Unitスロットの後ろも引ける）", run("s : [\n\ta : 1\n\tb : __\n\tc : 3\n]\ns ' c"), 3);
check("余積は逆に Unit を消す: |1 __ 2| → 2", run("|1 __ 2|"), 2);
