...
// **空は数えられない。** `[] ⇒ __` は潰れる向きで、戻せない（unit.md）——`__` を見て
// 「器だった」とは言えないので、0 と答えるのは逆写像の捏造である。**無かったものから
// 使える値を作る**ことになり、それは `null` が生まれた道と同じ形になる。実害は
// `||zzz||`（未定義の名前）が 0 を返して「空だった」と読めてしまうことだった。
checkTrue("空は数えられない（解なし）", value("||__||") === "__" && value("||[]||") === "__");
...
// 切り出しの `len` は 0 で頭打ちになるが、**`len = 0` は器ではなく `__` そのもの**なので、
// それを数えれば解なしである（上の「空は数えられない」と同じ規準）。
checkTrue("越えて切れば数えられない", value("f : s ? ||s ' 9~||\nf `abcde`") === "__");
