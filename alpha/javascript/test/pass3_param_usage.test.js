...
const cases = [
	...
	},
	{
		source: "f : x y ? 1 + y",
		want: { "<y>": "Address" },
		note: "右辺としての使用（1 + y の y）でも同じ。使われていない x は結果に含まれない",
	},
	// 相手のリテラルが型を決める。Sign には型注釈の構文が無いので（§1「型はコードの影」）、
	// 初期化時に型を決めたいときは**値を変えない演算**を書く——`+ 0` は Address、
	// `+ 0.0` は Float。実行時コストは無い（コンパイル時に消える）が型は固定される。
	{
		source: "f : x ? x + 0.0",
		want: { "<x>": "Float" },
		note: "`+ 0.0` は恒等演算だが型を Float に固定する（キャスト情報として働く）",
	},
	{
		source: "f : t ? t = `abc`",
		want: { "<t>": "String" },
