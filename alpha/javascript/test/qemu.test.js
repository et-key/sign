...
// 形でしか「書いてから読む」を書けない。
{
	...
	// **`$x` は `[x]` と同値である。** `$` は「型では無償な持ち上げ（`[x] ≅ x`）を、
	// 器の表現のまま実体化する」演算子である（原理8：無償なのは型の上だけで、表現には
	// 場所が要る）。だから長さは 1、`' 0` で中身が出て、`' 1` は範囲外——そして
	// **`@e` と `e ' 0` は同じことを言っている**。
	//
	// 名前付きの束縛にも場所が要る。演算子表が `$名前` を「binding 自体のアドレス。
	// binding ごとに一意・安定」と定めているのに、トップレベルの定数は命令へ畳まれて
	// 指す先が無かった——`$関数` は単相化が、`$匿名式` は `alloca` が扱えるのに、
	// **値の束縛だけが場所を持たなかった**。`.rodata` に置く。
	agree("$x を読む", "x : 7\n@$x");
	agree("$x を ' 0 で引く", "x : 7\n($x) ' 0");
	agree("$x の長さは1", "x : 7\n||$x||");
	agree("$x の範囲外", "x : 7\n($x) ' 1");
	agree("番地の束縛を読む", "y : 0x1234\nf : p ? @p\nf $y");
	// binding ごとに一意・安定：別の名前は別の場所、同じ名前は同じ場所。
	machineIs("別の束縛は別の場所", "a : 1\nb : 2\n($a) - ($b)", "-8");
	machineIs("同じ束縛は同じ場所", "a : 1\n($a) - ($a)", "0");
