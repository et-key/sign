...
// 書き写すのは**宣言順**である。物理配置は名前でソートした正規順（stack_abi.md §7.1）
// だが、それは宣言順から `sort` で導出できる。逆は導出できない。したがって宣言順の方が
// 情報として厳密に大きく、ソート順を書き写すとねじれ（宣言順と正規順の置換）が失われる。
//
// `point` と `point2` は `==` では等しい（Hom集合の一致は宣言順を問わない）が、連番で
// 引けば違う値を返す。それは `==` が比較していない性質を測っているだけであり、矛盾では
// ない。`.st` がソート順しか書かないとこの差が消え、「正した」事実だけが残って「何を
// 正したか」が暗黙化してしまう。
check("名前付きスロットは宣言順で名前を並べる", entries("p : [\n\tx : 1\n\ty : 2\n]"), ["p : Struct{x y}"]);
check("宣言順が違えば違う型として書き写される（ねじれが保存される）", entries("p : [\n\ty : 2\n\tx : 1\n]"), [
	"p : Struct{y x}",
]);
