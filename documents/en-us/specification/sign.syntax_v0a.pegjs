...
Output
  = target:Compute calls:(__ "#" __ Lambda)+ {
      return { type: "Output", target: target, calls: calls.map(t => t[3]) };
    }
  / Consideration
...
number = val:$("-"? int_part:[0-9]+ "."? frac_part:[0-9]*) { return val; }
...
// "Indent"と"Dedent"は、lexerがタブ文字の深さを見て挿入する仮想トークンです。
