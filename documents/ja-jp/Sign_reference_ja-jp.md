...
1. ### ? マークを使う関数定義方法    この方法は、以下に示す型で表される。
   ...
   * （例３　条件分岐をする関数を定義したい場合、比較演算も可能なmatch_caseの書き方）
    ```javascript
    ABS : x ?
    	x >= 0 : x
    	x < 0 : -x
   ...
   * （例６　再帰と条件分岐を同時に使う例、比較演算も可能なmatch_caseを含む）
     ```javascript
     collatz : x ?
    	x = 1 : `OK`
    	x % 2 = 0 : collatz x / 2
...
2. ### ポイントフリースタイルによる記述    この方法は、演算子を直接関数として扱う方法を提供するが、関数適用を挟むことになるため、演算子の優先順位がなくなることに注意！    型は以下の通り
   ...
   * 単項演算子のポイントフリー化は、演算子の前後に`_`をつけて表現する
     （例）
     ```javascript
     [!_] 5 = !5
     [_!] 5 = 5!
...
ここからは、比較演算子の説明となる。
Signでは、比較演算子は条件が真のときに特定の値を返し、偽のときに `_` (Unit) を返します。これにより、数学的かつ直感的な比較演算の連鎖（Chaining）を無矛盾に表現・評価します。

### 比較演算の値返却規則（値ベースの代数的ルール）
比較演算子（`<` など）に左辺（LHS）と右辺（RHS）を適用した際、真の場合に返却される値は**左辺の評価値**に基づいて決定されます。
- **左辺が単位元である場合**: 左辺値が `0`（加法単位元）、`1`（乗算単位元）、`_`（Unit）のいずれかである場合、**右辺（RHS）の値**を返します。
- **左辺が非単位元である場合**: それ以外の場合、**左辺（LHS）の値**を返します。

### 連鎖比較（三項比較など）の評価フロー（初項・次項ルール）
比較演算子を `a < b < c` のように連鎖させた場合、左結合（`(a < b) < c`）で以下のように評価されます。
1. **初項（最初の比較）**: 左辺 `a` が単位元（`0`, `1`, `_`）であれば右辺 `b` を返し、そうでなければ `a` を返します。
...
- `4 < 10 < 5` の場合、`(4 < 10)` は `10` を返し、`(10 < 5)` は偽となるため `_` を返します（全体として偽/falsyになります）。

less は、左のほうが小さければ真になり、偽なら `_` を返します。
...
「`@`前置演算子」や「`#`中置演算子」と組み合わせることで、ポインタ（レジスタ間接アドレッシング）を可能にする。
例
```
i : `hello`
@$i = i
...
### 左辺優先キャスト規則の厳密化（算術逸脱時の型エラー化）
二項演算において、Signは「左辺の型が全体の型を決定する」という原則（左辺優先キャスト）を持ちます。
しかし、左辺が文字列である `` `123` + 0 + 57 `` のような式において、算術演算を継続しようとするなど、算術ドメインから逸脱した結果が生じる場合は、暗黙の変換を行わず**型エラー（Type Error）**として処理されます。
これにより、暗黙のキャストによる予期しないバイトリスト等への変換を防ぎ、型システムの一貫性と堅牢性を保証します。
