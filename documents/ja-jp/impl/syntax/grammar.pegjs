...
// 後置演算子（@ ~ !）か空白のときだけ式になる。それ以外はすべてコメントである。**
//
// 行頭から文字列で始まる式は、インポート（`\`path`\`@~`）か連結（空白は段10 の余積）である。
// 散文の中では最初の閉じの直後は引用したい中身（非空白）なので、そこで分かれる。
// これでコメントの中にバッククォートを書ける——言語の構文を説明するのがコメントの
// 仕事である以上、その構文を引用できないのは実際に困る。
//
// 否定先読みの中でバッククォート自身を文字クラスから除くのが要である。PEG の `*` は
// バックトラックしない（貪欲に確定したら戻らない）ため、除かないと走査が行末まで
// 食い尽くし、閉じがそこにあっても「閉じは無い」と誤判定し続ける。
comment = SOL "`" !([^\r\n`]* "`" [@~! ]) [^\r\n]* (EOL / EOF) { return null; }
