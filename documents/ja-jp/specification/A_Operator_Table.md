...
| 24 | `@` | 中置※ | get | at（〜において） | 構造から値を取得 |
| 25 | `@` | 前置 | input | at（〜において） | アドレスからデータを取得 |
| 26 | `<<` | 中置 | 左ビットシフト | 左ビットシフト | 左ビットシフト |
| 26 | `>>` | 中置 | 右ビットシフト | 右ビットシフト | 右ビットシフト |
| 27 | `\|\|` | 中置 | bit or | ビットマスク | ビット毎の論理和 |
| 28 | `;;` | 中置 | bit xor | ビットマスク | ビット毎の非交和 |
| 29 | `&&` | 中置 | bit and | ビットマスク | ビット毎の論理積 |
| 30 | `!!` | 前置 | bit not | ビット反転 | ビット毎の否定 |
| 31 | `@` | 後置 | import | at（〜から） | ファイルから取得 |
| 32 | `(...)` | 囲み | block | ブロック | インラインブロック構築 |
| 32 | `{...}` | 囲み | block | ブロック | インラインブロック構築 |
| 32 | `[...]` | 囲み | block | ブロック | インラインブロック構築 |
| 32 | `	` | 前置 | indent | インデント | インデントブロック構築 |
| 33 | `\` | 前置 | escape | 文字化 | 直後の1文字を文字として扱う |
...
| `$...$` | メタ記述内Sign記述 | メタ記述内でのSign記述 | メタ記述内にSignの式を挿入 |
