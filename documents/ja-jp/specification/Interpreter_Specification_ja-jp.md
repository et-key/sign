...
### 評価戦略における `~`（後置演算子）の操作的意味論

Sign言語の根幹である「Pull-basedの遅延評価パラダイム」において、後置の `~` 演算子は、システムにおける最も重要な**シンク（評価の終端・トリガー）**として機能する。
この演算子は、インタプリタおよびコンパイラレベルで以下の2つの厳密な作用（セマンティクス）を持つ。

1. **評価の強制 (Forced Evaluation)**
   遅延ジェネレータ（例: `2項指定の ~+` など）や、未評価の関数ツリー（Thunk）に対し、値の確定を要求する（Pullする）。`~` が適用された時点で、計算グラフが実体化される。

2. **リストの平坦化 (Flattening)**
   対象が多次元リストやネストされた構造（辞書型など）である場合、WASM実装などにおける `$list_flatten` 命令に相当する処理が走り、一段階フラットな1次元のストリーム（または要素の羅列）に展開（コピー）される。

「展開作用によってオブジェクトがコピーされる」という高レベルな挙動は、この「評価の強制」と「平坦化」という2つの低レイヤーな作用の組み合わせによって実現されている。
