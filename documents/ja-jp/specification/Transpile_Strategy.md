...
## 1.5 Sign言語の基本原理と変換戦略の関係

### トークン判定の三段階原則

Sign言語のトランスパイル戦略は、言語の核心原理に直接基づいています:

| 段階 | 判定基準 | 変換での扱い | 実装箇所 |
|------|---------|-------------|---------|
| **第一** | 文字列か？ | プレースホルダーで保護 → 最終段階で復元 | [2] リテラル保護 |
| **第二** | 文字か？ | プレースホルダーで保護 → 最終段階で復元 | [2] リテラル保護 |
| **第三** | リテラルか記号か？ | リテラルは保持、記号は演算子として変換 | [5]-[9] 各種変換 |
...
この基本原理により、複雑なASTパーサーなしでの変換が可能となっています。
