...
> `__` が算術演算に出会っても `0` に「なる」のではなく、吸収元としてそのまま伝播する。
> ```sign
> __ + 2 → __   ` ← 2 にはならない。__ が算術層を超えて伝播する
> 0 + 2  → 2    ` ← 0 は算術ドメイン内のローカルな単位元
> ```
...
> 算術演算と出会っても `__` のまま吸収し、`0` や `1` への写像は行わない。
> この非対称性が `__` を「真偽値と等価」かつ「数値とは非等価」にしている根拠であり、
> Boolean型を必要としない理由でもある。

* **加法 `+`**:
  余積（スペース ` `）の**数値ドメインへの導来関手**。数値ドメイン内では加法単位元 `0` を持つが、これは `__` の写像ではなく、数値ドメイン固有のローカルな単位元である。`__` が `+` の引数に現れた場合は算術演算に入らず、そのまま吸収（`__ + x → __`）する。
* **乗算 `*`**:
  直積（カンマ `,`）の**数値ドメインへの導来関手**。数値ドメイン内では乗法単位元 `1` を持つが、同様に `__` の写像ではない。`__` が `*` の引数に現れた場合は吸収（`__ * x → __`）する。
* **比較演算 `<`**:
...
#### 3.5.4 実行時セマンティクス：非対称な零射伝播と単位律

実行時のプログラム評価において、`__` の伝播は左右非対称であります（詳細は [A_Operator_Table.md](../syntax/A_Operator_Table.md) 「非対称 Unit 伝播則」を参照）。

$$ f(\_\_, x) = \_\_ \qquad f(x, \_\_) = x $$

- **左辺が `__`**：即座に式全体が `__` に崩壊します（吸収元 / 零射の合成）。例：`__ + 5 = __`、`__ * 5 = __`
- **右辺が `__`**：左辺の値 `x` がそのまま返ります（単位元 / 単位律）。例：`5 + __ = 5`、`5 * __ = 5`

> [!NOTE]
> **例外：対称吸収を行う演算子**
> 論理積 `&`（優先順位7）および比較演算子 `<`, `<=`, `=`, `>=`, `>`（優先順位12、`!=` を除く）は、左右どちらが `__` でも即座に `__` へ崩壊する「対称吸収」を行います。この2種の演算子群についてのみ $f(x, \_\_) = f(\_\_, x) = \_\_$ が成立します。
