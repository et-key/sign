...
### 3.3 Unit の型規則（非対称 Unit 伝播則）

`__`（Unit）は、カテゴリの構造を定義する「圏論的構成（積・余積）」においては双対的な役割を担いますが、`+` や `<` のような**「射の構成子（単なる射）」**においては、式の左右で振る舞いが変わる「非対称 Unit 伝播則」を持ちます。
...
|:---:|:---:|---|---|
| `&`（AND）| 積 | 吸収元: `__ & x = __` | 吸収元: `x & __ = __` |
| `\|`（OR） | 余積 | 単位元: `__ \| x = x` | 単位元: `x \| __ = x` |
| `;`（XOR）| 余積対称差 | 単位元: `__ ; x = x` | 単位元: `x ; __ = x` |
| `+`, `*`, `<`, `!=` 等 | 射の構成子 | 吸収元（零射）: `__ + x = __` | **単位元（id射）: `x + __ = x`** |

この**「右辺の Unit は単位元として無視される（`x op __ = x`）」**という仕様は、再帰関数などで可変引数（`~rest`）が尽きた際も、余分な終了条件を書かずに自然に計算が底打ちするように設計されているためです。
