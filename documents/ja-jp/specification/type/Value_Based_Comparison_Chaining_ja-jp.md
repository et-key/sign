...
* **単位元（例外ルール）**: 左辺の**Int 値**が `0`（加法単位元）、`1`（乘算単位元）のいずれかである場合、**右辺（RHS）の値**を返します。
* **通常ルール**: それ以外の場合、**左辺（LHS）の値**を返します。（余積単位元 `__` の場合は、算術演算の単位元ではないため、左辺の値を返します。）

> [!IMPORTANT]
> **`{0, 1}` の判定は Int ドメインの算術単位元のみを対象とする。**
>
> | 値 | ドメイン | `{0,1}` 判定対象 |
> |---|---|:---:|
> | `0`, `1`（Int） | Int（整数） | ✅ |
> | `0.0`, `1.0`（Float） | f64 | ❌（false ではない） |
> | `0r00`（raw binary） | isize | ❌（アドレス値として使うとエラー） |
> | `[0]`（List） | list(0) | ❌（値ではなくリスト） |
> | `\a`（文字型） | 単一文字 | ✅（文字コードが 0 または 1 の場合） |
> | `0u0000`（Unicode型） | Unicodeコードポイント | ✅（値が 0 または 1 の場合） |
>
> Float・リスト・アドレス型は数値的に 0 であっても判定対象外。
>
> **`==` / `!==`（構造比較）は本ルールの適用外。**
