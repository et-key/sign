...
_start:
    ...
    ADRP x1, Sign_Str_0purs
    ADD  x1, x1, :lo12:Sign_Str_0purs
    MOV x2, #0x0000
    MOVK x2, #0x4024, LSL #48
    ADRP x3, Sign_Str_k9mnw
    ADD  x3, x3, :lo12:Sign_Str_k9mnw
    MOV x4, #0x0000
    ...
    MOV x4, x5
    LDR x4, [fp, #-8] // Load 'd'
    ADRP x5, Sign_Str_524zd
    ADD  x5, x5, :lo12:Sign_Str_524zd
    LDR x2, [x4, #0] // TypeTag
    MOV x1, #0xFFFD
    ...
    CMP x2, x1
    B.EQ .L_is_dict_b54xy
    MOV x3, x28 // Not a dict -> Unit
    B .L_get_str_end_klng6
.L_is_dict_b54xy:
    LDR x1, [x4, #8]
    MOV x6, #0
    ADD x7, x4, #16
.L_dict_loop_55lp5:
    CMP x6, x1
    B.GE .L_dict_not_found_idlyh
    LDR x9, [x7] // dict key ptr
    LDR x10, [x9]
    LDR x11, [x5]
    MOV x13, #0
    CMP x10, x11
    B.NE .L_strcmp_end_w6id9
    MOV x12, #0
.L_strcmp_loop_xzt5x:
    CMP x12, x10
    B.GE 1f // match!
    ...
    LDR x14, [x5, x2]
    CMP x11, x14
    B.NE .L_strcmp_end_w6id9 // char differs
    ADD x12, x12, #1
    B .L_strcmp_loop_xzt5x
1:
