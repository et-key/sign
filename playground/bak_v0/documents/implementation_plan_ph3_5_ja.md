...
- **単体の演算子評価（例: `[ 10 + - * ]` における演算子の孤立）:**
  - パース結果のAST構造上、値が期待される場所（識別子としての評価時）にネイティブ演算子が出現した場合、意図しないスタック消費を防ぐため、WASMの直接的な演算命令は出力しません。
  - 代わりに、**その演算子が必要とする引数分のダミーを受け取る「カリー化されたクロージャ（第一級関数）」を動的に生成するASTを構築し、関数ポインタとしてスタックに積みます。**
  - これにより、ネイティブ演算子であってもSignの純粋関数型の哲学に沿った部分適用（クロージャの返却）がシームレスに機能し、WASMのスタック整合性も完全に保たれます。
...
> **[Update: 最新仕様 (ブロックの逐次評価化)]**
> ブロック構文の評価規則は「短絡OR / パイプライン連鎖」から**「純粋な手続き的逐次実行（モナディックなシーケンス）」**へと変更されました。各行は上から順に独立して評価・破棄（`drop`）され、最後の式の値がブロック全体の戻り値となります。
...
> **[Update: 解決済み]** ブロックが逐次評価に変更され、`:`（Guard）による明示的なジャンプ命令がない限り早期リターン（短絡評価）は発生しなくなったため、この致命的な競合問題は仕様レベルで解消されました。
