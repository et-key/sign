...
- どちらもベースは同じヒープ上の構造体ですが、以下のレイアウトで保存されます。
  ...
  > **[Update: 最新仕様 (動的辞書アクセス)]**
  > Hidden Classパターンの導入によりレイアウトが変更されました。**オフセット `+0` にはShape/メタデータ（キー文字列配列等）へのポインタ**が格納され、実際のプロパティ値は `+8`, `+16`... とシフトして配置されます。
...
> **[Update: 最新仕様 (Zero Cost Domain Abstractionの拡張)]**
> 静的なオフセット解決を基本としますが、コンパイル時にキーが決定できない場合（動的な文字列アクセス等）は、WASMランタイムの動的検索関数（`$dynamic_dict_get`）にフォールバックする仕様に進化しました。静的解決が可能な場合はこれまで通り実行時オーバーヘッドはゼロです。
