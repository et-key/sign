...
> **[Update: 最新仕様 (ハックの削除とMatchCaseの導入)]**
> ブロックの意味論が「逐次実行（WASMの `drop` による破棄）」に正式変更されたため、上記の `NaN` を強制返却する苦し紛れのハックは完全に削除されました。現在の `:` はSignの「同一視」の哲学に従い、ストアした値をそのままスタックに残します。条件分岐（早期リターン）は、左辺が式である場合の `:`（MatchCase Guard）でのみ、WASMの `br` 命令として明示的に発動します。
