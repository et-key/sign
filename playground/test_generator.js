...
function _call(fn, ...args) {
  ...
  if (totalArgs.includes(__unit)) return __unit;
...
const _sig_map = (() => {
  const _fn = (_sig_g, _sig_x, ..._sig_y) => {
  if (_sig_y.length === 0) _sig_y = __unit;
  return [(_call(_deref(_sig_g), _sig_x)), (_call(_sig_map, _sig_g, new ExpandedStream(_expand(_sig_y))))];
};
  _fn.expectedLength = 3;
  _fn.requiredLength = 2;
  _fn.hasRest = true;
  _fn.paramSpecs = [{"name":"_sig_g","defaultValue":null,"isRest":false},{"name":"_sig_x","defaultValue":null,"isRest":false},{"name":"_sig_y","defaultValue":null,"isRest":true}];
  _fn._extractIndex = -1;
  return _fn;
})();
((_a0) => (_a0 === __unit) ? __unit : _sig_say(_a0))((((_a0, _a1) => (_a0 === __unit || _a1 === __unit) ? __unit : _sig_map(_a0, _a1))(new Address(_makePointFreeMapFilter(((x) => _arithmetic('*', x, 2)), false)), (_range(0, 10, 2, 'arithmetic')))));
