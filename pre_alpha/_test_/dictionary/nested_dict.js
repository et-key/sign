...
const type = __unit;
const backend = __unit;
const option = (() => {
  const name = `Sign`;
  const version = 0.1;
  const type = `Structural`;
  const backend = `AArch64`
  return { name: name, version: version, type: type, backend: backend };
})();
const data = (() => {
  const type = `Structural`;
  const backend = `javascript`
  return { type: type, backend: backend };
})();
const update = (() => {
  const _fn = (_d0, _d1) => {
  ...
  if (option.length === 0) option = __unit;
  const _d1_val = _expand(_d1);
  let type, backend;
  if (Array.isArray(_d1_val) && _d1_val.length === 1 && _d1_val[0] && typeof _d1_val[0] === 'object' && _d1_val[0].constructor.name === 'ExpandedObject') {
    const _dict = _d1_val[0].obj;
    type = ('type' in _dict) ? _dict['type'] : __hole;
    backend = ('backend' in _dict) ? _dict['backend'] : __hole;
  } else if (Array.isArray(_d1_val)) {
    [type, backend] = _d1_val;
  } else {
    type = _d1_val;
  }
  return (() => {
  return _overwrite(option, ((() => {
  _overwrite(new Address(type), type);
  return _overwrite(new Address(backend), backend);
})()));
})();
};
  ...
  _fn.hasRest = false;
  _fn.paramSpecs = [{"name":"_destruct_pattern","defaultValue":null,"isRest":false,"isDestructured":true,"innerSpecs":[{"name":"option","defaultValue":null,"isRest":true}]},{"name":"_destruct_pattern","defaultValue":null,"isRest":false,"isDestructured":true,"innerSpecs":[{"name":"type","defaultValue":null,"isRest":false},{"name":"backend","defaultValue":null,"isRest":false}]}];
  _fn._extractIndex = undefined;
  return _fn;
})();
_call(update, option, data);
