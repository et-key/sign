...
const func_match = (() => {
  const _fn = (x, y) => {
  return (() => {
  if (_isTrue(_compare('<', x, 1))) return `result1`;
  if (_isTrue(_compare('<', y, 3))) return `result2`;
  return _concat(``, _arithmetic('*', x, y));
})();
};
...
try { console.log("func_match = ", util.inspect(func_match, { depth: null, colors: true })); } catch(e) {}
