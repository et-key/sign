...
function shuntingYard(tokens) {
    ...
    const applyOp = (opToken) => {
        ...
        const isCompare = ['<', '>', '<=', '>=', '=', '!='].includes(opValue);

        if (notation === 'infix') {
            let right = values.pop();
            let left = values.pop();

            // Ternary Flattening Logic
            if (isCompare && Array.isArray(left) && left[0] === opValue) {
                // Detected A < B ... < C
                // Left is [ "<", A, B ]
                // We want [ "<", A, B, Right ]
                left.push(right);
    ...
    for (let t of tokens) {
        if (isVal(t)) {
            ...
            // Ternary Comparison Check (Lookahead)
            // If current op is comparison and previous op on stack (top) is SAME comparison,
            // and we haven't reduced it yet, we might want to flatten?
            // Actually, Shunting Yard usually reduces immediately.
            // But for A < B < C:
            // 1. push A
            // 2. push <
            // 3. push B
            // 4. see < (second one).
            //    prec is same. assoc is left.
            //    Normally pops first <. applies [ < A B ].
            //    Then pushes second <.
            //    Result: [ < [ < A B ] C ]
            // We want [ < A B C ].

            // To achieve this, when we see second <, if top of stack is < (SAME op),
            // instead of applying/reducing, we might want to fuse?
            // "Association: None" or custom handling.

            // If explicit ternary support:
            // When applying op, check if left operand is ALREADY a node with SAME op?
            // [ < A B ]
            // New op < comes.
            // Right is C.
            // We form [ < [ < A B ] C ].
            // We can post-process or handle in `applyOp`.
...
// Helper to flatten ternary or chain comparisons if they are same operator
function applyOp(opToken, values) { // Modified signature to pass values explicitly? 
    // Wait, shuntingYard closure context has `values`. I need to edit `applyOp` inside `shuntingYard`.
}
...
export { tokenize, structurize, parseBlock, resolveTokens };
