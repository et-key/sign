...
const { OperatorList } = require('../../utility/operators.js');
...
// OperatorList is [{ op: { ... } }, ...]
const allOperators = OperatorList.map(obj => Object.keys(obj)[0]);
// Unique and sorted
    ...
    tokenize() {
        for (let i = 0; i < this.lines.length; i++) {
            ...
            while (cursor < content.length) {
                ...
                // 6. Identifiers
                match = rest.match(/^[^\s()\[\]{},:?"`]+/); // Basic loose identifier pattern
                if (match) {
