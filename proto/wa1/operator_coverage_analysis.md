...
| 優先順位 | 記号 | 機能 | 状態 |
|---------|------|------|------|
| 24 | `<<` | 左ビットシフト | ✅ 実装済み (I32_SHL) |
| 24 | `>>` | 右ビットシフト | ✅ 実装済み (I32_SHR_S) |
| 25 | `\|\|` | bit or | ✅ 実装済み (I32_OR) |
| 26 | `;;` | bit xor | ✅ 実装済み (I32_XOR) |
| 27 | `&&` | bit and | ✅ 実装済み (I32_AND) |
| 28 | `!!` | bit not | ✅ 実装済み (I32_XOR with -1) |
...
8. ~~**ビット演算子**: `<<`, `>>`, `||`, `;;`, `&&`, `!!`~~ ✅ 完了
   - WebAssembly の i32 ビット演算命令を使用 (実装済み)
9. **エクスポート演算子**: `#`, `##`, `###`
   - モジュールシステムの実装が必要
10. **ブロック構文**: `(...)`, `{...}`
    - 現在はインデントとブラケットのみ対応
