...
| 優先順位 | 記号 | 位置 | 機能 | 実装状況 |
|---------|------|------|------|---------|
| 9 | `~` | 中置 | range | Parser: 認識、Compiler: 未実装 |
...
| 優先順位 | 記号 | 機能 | 状態 |
|---------|------|------|------|
| 11 | `;` | xor | Parser: operators.js に定義、Compiler: 未実装 |
| 11 | `\|` | or | Parser: `\|` として認識 (絶対値囲みとして実装)、Compiler: 論理和未実装 |
| 12 | `&` | and | Parser: operators.js に定義、Compiler: 未実装 |

#### 指数・階乗演算子
| 優先順位 | 記号 | 位置 | 機能 | 状態 |
|---------|------|------|------|------|
| 17 | `^` | 中置 | pow | Parser: operators.js に定義、Compiler: 未実装 |
| 18 | `!` | 後置 | factorial | Parser: `_!` として認識、Compiler: 未実装 |

#### 絶対値演算子
| 優先順位 | 記号 | 位置 | 機能 | 状態 |
|---------|------|------|------|------|
| 19 | `\|...\|` | 囲み | abs | Parser: `[\|` と `\|]` として実装、Compiler: 絶対値演算未実装 (現在は `!-` ノードを生成) |
...
**最近の進捗 (2026-02-03)**:
- 優先度「高」の演算子 (`&`, `|`, `;`, `^`, `|...|`) を実装完了
- Wasm Import Section のサポート追加
- 実装率が 23% → 31% に向上

**今後の課題**:
仕様に定義されている演算子の約69%はまだ未実装であり、特に以下の領域が不足しています:

- 範囲・リスト生成演算子 (優先度: 中)
- アクセス・構造操作演算子 (優先度: 中)
- ビット演算子 (優先度: 低)
- モジュール・エクスポート機能 (優先度: 低)

今後の開発では、優先度「中」の演算子を段階的に実装していくことで、仕様への準拠度をさらに高めることができます。
